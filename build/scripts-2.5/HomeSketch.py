""" HomeSketch
    ...
    An Home Layout sketching program by TransceiveTechnology.

    #########################################################################

    NOTE

    HomeSketch requires wxPython version 2.3.  If you are running an earlier
    version, you need to edit your copy of wxPython/windows.py to fix a bug
    which will otherwise cause the "Edit Text Object" dialog box to crash.

    Open the wxPython/windows.py file and find the wxPyValidator.__init__ 
    method and change the line self._setSelf(self, wxPyValidator, 0)
    to self._setSelf(self, wxPyValidator, 1)
    ...
    Known Bugs:
      ...
      * Scrolling the window causes the drawing panel to be messed up until you
...
class DrawingFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title, fileName=None):
        ...
        self.helpMenu.Append(menu_ABOUT, "About HomeSketch...")
        ...
        self.Bind(wx.EVT_TIMER, self.onIdle) #odd that you need to call the repaint/idle function using a timer
        ...
        self._setPenColor(wx.BLACK)
        self._setFillColor(wx.Color(215,253,254))
        self._setLineSize(2)
    ...
    def OnClose(self, event):
      print "HomeSketch.DrawFrame.OnClose() called"
    ...
    def onPenOptionIconClick(self, event):
        ...
        if len(self.selection) == 1:
            data.SetColor(self.selection[0].getPenColor())
        else:
            data.SetColor(self.penColor)

        dialog = wx.ColorDialog(self, data)
        dialog.SetTitle('Choose line color')
        if dialog.ShowModal() == wx.ID_OK:
            c = dialog.GetColorData().GetColor()
            self._setPenColor(wx.Color(c.Red(), c.Green(), c.Blue()))
        dialog.Destroy()
    ...
    def onFillOptionIconClick(self, event):
        ...
        data = wx.ColorData()
        if len(self.selection) == 1:
            data.SetColor(self.selection[0].getFillColor())
        else:
            data.SetColor(self.fillColor)

        dialog = wx.ColorDialog(self, data)
        dialog.SetTitle('Choose fill color')
        if dialog.ShowModal() == wx.ID_OK:
            c = dialog.GetColorData().GetColor()
            self._setFillColor(wx.Color(c.Red(), c.Green(), c.Blue()))
        dialog.Destroy()
    ...
    def doChooseQuality(self, event):
        ...
        if event.GetId() == menu_DC:
            self.wrapDC = lambda dc: dc # so the new function is def self.wrapDC(x):\n return x #(do nothing)
            ...
            self.wrapDC = lambda dc: wx.GCDC(dc) # so the new function is self.wrapDC(x):\n return wx.GCDC(x) #(wrap the argument in wx.GCDC)
        self._adjustMenus()
        self.requestRedraw()
    ...
    def doShowAbout(self, event):
        """ Respond to the "About HomeSketch" menu command.
        """
        dialog = wx.Dialog(self, -1, "About HomeSketch") # ,
                          #style=wx.DIALOG_MODAL | wx.STAY_ON_TOP)
        dialog.SetBackgroundColor(wx.WHITE)

        panel = wx.Panel(dialog, -1)
        panel.SetBackgroundColor(wx.WHITE)
        ...
        lab1 = wx.StaticText(panel, -1, "HomeSketch")
        ...
        lab2 = wx.StaticText(panel, -1, "A home layout sketch pad by Transceive Technology")
        lab2b = wx.StaticText(panel, -1, "(C) 2010, All Rights Reserved.")
        lab2.SetFont(boldFont)
        lab2.SetSize(lab2.GetBestSize())

        lab3 = wx.StaticText(panel, -1, "HomeSketch is proprietary software by Transceive Technology")
    ...
    def _setPenColor(self, color):
        """ Set the default or selected object's pen color.
        """
        if len(self.selection) > 0:
            self.saveUndoInfo()
            for obj in self.selection:
                obj.setPenColor(color)
            self.requestRedraw()

        self.penColor = color
        self.optionIndicator.setPenColor(color)


    def _setFillColor(self, color):
        """ Set the default or selected object's fill color.
        """
        if len(self.selection) > 0:
            self.saveUndoInfo()
            for obj in self.selection:
        ...
        self.optionIndicator.setFillColor(color)
...
class RectDrawingTool(DrawingTool):
    ...
    def onMouseMotion(self,parent, event):
        ...
        if self.newObject is None:
            obj = RectDrawingObject(penColor=parent.penColor,
                                    fillColor=parent.fillColor,
                                    lineSize=parent.lineSize)
            self.newObject = obj
...
class EllipseDrawingTool(DrawingTool):
    ...
    def onMouseMotion(self,parent, event):
        ...
        if self.newObject is None:
            obj = EllipseDrawingObject(penColor=parent.penColor,
                                       fillColor=parent.fillColor,
                                       lineSize=parent.lineSize)
            self.newObject = obj
...
class PolygonDrawingTool(DrawingTool):
    ...
    def onMouseLeftDown(self,parent, event):
        ...
        if self.newObject is None:
            obj = PolygonDrawingObject(points=[(0,0)],penColor=parent.penColor,
                                       fillColor=parent.fillColor,
                                       lineSize=parent.lineSize,
                                       position=wx.Point(event.X, event.Y))
...
class ScribbleDrawingTool(DrawingTool):
    ...
    def onMouseLeftDown(self,parent, event):
        event.Skip()
        obj = ScribbleDrawingObject(points=[(0,0)],penColor=parent.penColor,
                                    fillColor=parent.fillColor,
                                    lineSize=parent.lineSize,
                                    position=wx.Point(event.X, event.Y))
        self.newObject = obj
...
class DrawingObject(object):
    """ Base class for objects within the drawing panel.
        ...
        A HomeSketch document consists of a front-to-back ordered list of
        DrawingObjects.  Each DrawingObject has the following properties:
            ...
            'penColor'     The color to use for drawing the object's outline.
            'fillColor'    Color to use for drawing object's interior.
    ...
    def __init__(self, position=wx.Point(0, 0), size=wx.Size(0, 0),
                 penColor=wx.BLACK, fillColor=wx.WHITE, lineSize=1,
                 ):
        """ Standard constructor.
    ...
    def setData(self, data):
        ...
        try:
            ...
            self.penColor         = wx.Color(red=d.next(),
                                              green=d.next(),
                                              blue=d.next())
            self.fillColor        = wx.Color(red=d.next(),
                                              green=d.next(),
                                              blue=d.next())
    ...
    def setPenColor(self, color):
        """ Set the pen color used for this DrawingObject.
        """
        self.penColor = color


    def getPenColor(self):
        """ Return this DrawingObject's pen color.
        """
        return self.penColor


    def setFillColor(self, color):
        """ Set the fill color used for this DrawingObject.
        """
        self.fillColor = color


    def getFillColor(self):
        """ Return this DrawingObject's fill color.
        """
    ...
    def draw(self, dc, selected):
        ...
        """
        if self.lineSize == 0:
            dc.SetPen(wx.Pen(self.penColor, self.lineSize, wx.TRANSPARENT))
        else:
            dc.SetPen(wx.Pen(self.penColor, self.lineSize, wx.SOLID))
...
class TextDrawingObject(DrawingObject):
    ...
    def draw(self, dc, selected):
        ...
        """
        dc.SetTextForeground(self.penColor)
        dc.SetTextBackground(self.fillColor)
...
class ToolOptionIndicator(wx.Window):
    ...
    def setPenColor(self, penColor):
        """ Set the indicator's current pen color.
        """
        self.penColor = penColor
        self.Refresh()
    ...
    def setFillColor(self, fillColor):
        """ Set the indicator's current fill color.
        """
        self.fillColor = fillColor
        self.Refresh()
...
class EditTextObjectDialog(wx.Dialog):
    ...
    def OnSelectFont(self, evt):
        ...
        data.SetColor(self.curClr)         # set color
...
class SketchApp(wx.App):
    """ The main HomeSketch application object.
    """
    def OnInit(self):
...
def main():
    """ Start up the HomeSketch application.
    """
    global _app
    ...
    # Create and start the HomeSketch application.
