...
class RadioObject(MovingGroup):
  ...
  def __init__(self, LabelText="A", XY=[0.0,0.0], WH=[2.0,2.0], Family=wx.SWISS, Weight=wx.BOLD, FillColor="Green", TextColor="Black",
    ...
    self.Label = FC.ScaledText(LabelText, XY=XY, Size=WH[1]/1.8, Family=Family, Weight=Weight, Color=TextColor, Position='cc')
    self.Ellipse = FC.Ellipse( (XY[0]-WH[0]/2.,XY[1]-WH[1]/2.), WH, FillColor=FillColor, LineStyle=LineStyle, LineWidth=LineWidth)
    MovingGroup.__init__(self, (self.Ellipse, self.Label), InForeground, IsVisible)
  def SetPosition(self,newXY=[0.,0.]):
    ...
    #self.Label.Move( self,np.asarray(newXY, np.float).reshape(2,)-self.BoundingBox.Center)
    #self.Ellipse.Move(self,np.asarray(newXY, np.float).reshape(2,)-self.BoundingBox.Center)
    self._Canvas.BoundingBoxDirty = True
...
class RadioPanel(NC.NavCanvas):
  ...
  def InitializeRadios(self):
    ...
    for i in range(self.N):
      a = i*np.pi*2/self.N
      self.RadioObjectArray.append(RadioObject( str(i), (np.cos(a),np.sin(a)), WH=(.25,.25), FillColor="Green", LineStyle="Solid", LineWidth=3))
  ...
  def AddRadio(self,position=None):
    ...
    self.RadioObjectArray.append(RadioObject( str(self.N), (position[0],position[1]), WH=(.25,.25), FillColor="Green", LineStyle="Solid", LineWidth=2, InForeground=True))
    #self.Canvas.clear()
    for i in range(self.N):
    ...
    #for r in RadioObjectArray:
    self.Canvas.AddObject(self.RadioObjectArray[-1])
    #Can't bind to hit test event until object has been added to Canvas
