...
class Controller(object):
    ...
    while self and not self.done: #alive: # what about isRunning() member function for apps?
      ...
      if not self.pause and not self.done:
        ...
        if p != None and p.t>0 and p.neighbors[0][1]>0 and self and not self.done:
          ...
          #dbgHL: The next line precipitates an Xwin error
  ...
  def OnPubMessage(self, msg):
      ...
      if "REFRESH" in msg.topic:
        ...
        #dbgHL: next line causes an Xwin error in Ubuntu, even when run with gksudo
        #"python: Fatal IO error 11 (Resource temporarily unavailable) on X server :0.0"
     ...
     # can't figure out how to pass differenct data to different attributs in the same object going through the same queue
      ...
      try:
        ...
        positions2view.task_done()
      	...
      	log.debug("OnPubMessage() positions2view.get_nowait() caused Queue.Empty exception.")
      	#pass
      except IOError:
      	log.debug("OnPubMessage() positions2view.get_nowait() caused IOError exception.")
      	#pass
      except Exception:
      	log.debug("OnPubMessage() positions2view.get_nowait() caused Exception exception.")
      if len(p2v):
        self.view.panel1.Moving = True # stop responding to user click/drags on objects
        self.pause = True # stop processing packets  
      	log.debug("OnPubMessage() self.view.SetPositions(p2v) command about to execute.")
      ...
      try:
        ...
        distances2view.task_done()
      	...
      	log.debug("OnPubMessage() distances2view.get_nowait() caused Queue.Empty exception.")
      if len(d):
